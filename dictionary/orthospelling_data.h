...
struct OrthospellingData {
  ...
  struct Definition {
    StenoStroke stroke;
    uint8_t priority; // 0-2 inclusive, representing the buffer used in
    const char data[8];
  };
  struct Exit {
    StenoStroke stroke;
  ...
  SizedList<Definition> definitions;
  SizedList<Exit> exits;

  // Returns nullptr if the stroke is not a starting stroke.
  const char *GetStarterDefinition(StenoStroke stroke) const;
